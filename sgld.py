...
def main(args):
    ...
    for idx, sigma in enumerate(sigmas):
        ...
        for i in range(n_steps):
            j = max(receptive_field,np.random.randint(x0.shape[1]-receptive_field))
            grad_window = min(j+SGLD_WINDOW, x0.shape[1])
            eval_window = min(j+SGLD_WINDOW+receptive_field, x0.shape[1])
            #print('coordinates: ', j, grad_window, eval_window)
            ...
            # Forward pass
            log_prob, prediction0 = model0.smoothed_loss(x0[:,j:eval_window], sigma=sigma)
            log_prob0 = torch.sum(log_prob)
            grad0 = torch.autograd.grad(log_prob0, x0)[0]
            x0_update = eta * grad0[:, j:grad_window]

            log_prob, prediction1 = model1.smoothed_loss(x1[:,j:eval_window], sigma=sigma)
            log_prob1 = torch.sum(log_prob)
            grad1 = torch.autograd.grad(log_prob1, x1)[0]
            x1_update = eta * grad1[:, j:grad_window]

            # Langevin step
            epsilon0 = np.sqrt(2 * eta) * torch.normal(0, 1, size=(1, grad_window-j), device=device)
            x0_update += epsilon0

            epsilon1 = np.sqrt(2 * eta) * torch.normal(0, 1, size=(1, grad_window-j), device=device)
            x1_update += epsilon1

            # Reconstruction step
            x0_update -= eta * gamma * (x0[:, j:grad_window] + x1[:, j:grad_window] - mixed[:, j-receptive_field:grad_window-receptive_field])
            x1_update -= eta * gamma * (x0[:, j:grad_window] + x1[:, j:grad_window] - mixed[:, j-receptive_field:grad_window-receptive_field])

            with torch.no_grad():
                x0[:, j:grad_window] += x0_update
                x1[:, j:grad_window] += x1_update
            ...
            if (not i % 20) or (i == (n_steps - 1)): # debugging
                ...
                print("Max sample {}".format(
                    abs(x0).max()))
                print('Mean sample logpx: {}'.format(log_prob0 / SGLD_WINDOW))
                print('Mean sample logpy: {}'.format(log_prob1 / SGLD_WINDOW))
